...
func NewRouter() *web.Service {
	...
	jsr.DefaultOptions = append(
		jsr.DefaultOptions, jsonschema.InterceptDefName(
	...
	s.Post(
		"/json-body-manual/{in-path}", jsonBodyManual(),
		nethttp.SuccessStatus(http.StatusCreated),
	)
	...
	s.Get(
		"/error-response", errorResponse(), func(h *nethttp.Handler) {
		},
	)
	...
	for p, pi := range r.Spec.Paths.MapOfPathItemValues {
		pi.Parameters = append(
			pi.Parameters, openapi31.ParameterOrReference{
