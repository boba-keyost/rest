...
func Test_taskLifeSpan(t *testing.T) {
	...
	assert.NoError(
		t, rc.ExpectResponseBody(
			[]byte(`{"createdAt": "<ignore-diff>",`+
	...
	assert.NoError(
		t, rc.ExpectOtherResponsesBody(
			[]byte(`{"status":"ALREADY_EXISTS",`+
	...
	assert.NoError(
		t, rc.ExpectResponseBody(
			[]byte(`{"id":1,"goal":"string","deadline":"2020-05-17T11:12:42.085Z",`+
	...
	assert.NoError(
		t, rc.ExpectResponseBody(
			[]byte(`{"id":1,"goal":"foo","deadline":"2020-05-17T11:12:42.085Z",`+
				`"createdAt":"<ignore-diff>"}`),
		),
	)
	...
	assert.NoError(
		t, rc.ExpectOtherResponsesBody(
			[]byte(`{"status":"FAILED_PRECONDITION",`+
	...
	assert.NoError(
		t,
		rc.ExpectResponseBody([]byte(`{"id":"<ignore-diff>","goal":"goal!","deadline":"2022-06-20T23:49:10.227Z","createdAt":"<ignore-diff>"}`)),
	)
}
