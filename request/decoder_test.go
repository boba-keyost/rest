...
func TestDecoder_Decode(t *testing.T) {
	req, err := http.NewRequestWithContext(
		context.Background(), http.MethodPost, "/?in_query=abc",
	...
	df := request.NewDecoderFactory()
	df.SetDecoderFunc(
		rest.ParamInPath, func(r *http.Request) (url.Values, error) {
	...
	decCM := df.MakeDecoder(
		http.MethodPost, input, map[rest.ParamIn]map[string]string{
			rest.ParamInHeader: {
...
func BenchmarkDecoderFunc_Decode(b *testing.B) {
	...
	df := request.NewDecoderFactory()
	df.SetDecoderFunc(
		rest.ParamInPath, func(_ *http.Request) (url.Values, error) {
			return url.Values{"in_path": []string{"mno"}}, nil
		},
	)
...
func TestDecoder_Decode_json(t *testing.T) {
	req, err := http.NewRequestWithContext(
		context.Background(), http.MethodPost, "/?in_query=cba",
	...
	assert.Equal(
		t,
		rest.ValidationErrors{"body": []string{"#/bodyTwo: minimum 2 items allowed, but found 1 items"}},
		err,
	)
}
...
func TestDecoder_Decode_queryObject(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(
		t, rest.RequestErrors{
			"query:in_query": []string{
...
func TestDecoder_Decode_unknownParams(t *testing.T) {
	...
	assert.Equal(
		t, rest.ValidationErrors{"query:quux": []string{"unknown parameter with value 123"}}, err,
		fmt.Sprintf("%#v", err),
	)
}
